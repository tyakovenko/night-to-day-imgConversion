"""
Training script v2 — U-Net with improved loss functions.

Changes vs train_extended.py:
  - Loss: EnhancementLoss (L1 + MS-SSIM + VGG perceptual) instead of MSELoss
  - Crop size default: 256 (larger crops give MS-SSIM meaningful context)
  - Warm-start from best_extended.pt if it exists, else best.pt
  - Saves to checkpoints/best_v2.pt / last_v2.pt
  - Logs to experiment_log_v2.csv

Usage:
    python train_v2.py [--epochs 20] [--batch-size 4] [--crop-size 256]
                       [--lr 1e-5] [--perceptual-weight 0.1] [--workers 4]
                       [--init-checkpoint checkpoints/best_extended.pt]
                       [--resume checkpoints/last_v2.pt]
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Train U-Net v2 with L1 + MS-SSIM + VGG perceptual loss"
    )
    parser.add_argument("--epochs",             type=int,   default=20)
    parser.add_argument("--batch-size",         type=int,   default=4,
                        help="Default 4 — larger crops need more VRAM")
    parser.add_argument("--crop-size",          type=int,   default=256,
                        help="256 gives MS-SSIM enough spatial context (min 160)")
    ...
    parser.add_argument("--resume",             type=str,   default=None,
                        help="Resume v2 training from last_v2.pt")
    ...
    print(f"\n{'='*65}")
    print(f"  Low-Light Enhancement — v2 Training (L1 + MS-SSIM + VGG)")
    print(f"{'='*65}")
    ...
    if args.resume and Path(args.resume).exists():
        ...
        print(f"Resumed v2 training from {args.resume}  (epoch {ckpt['epoch']})\n")
    else:
    ...
    # Loss: EnhancementLoss (L1 + MS-SSIM + VGG perceptual)
    criterion = EnhancementLoss(perceptual_weight=args.perceptual_weight).to(device)
    ...
    # ── Experiment log ───────────────────────────────────────────────────────
    log_path   = Path("experiment_log_v2.csv")
    log_fields = [
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        # ── Save checkpoints (v2 names — originals untouched) ────────────────
        ckpt_state = {
            "epoch":        epoch,
    ...
    print(f"  Best checkpoint:   {ckpt_dir}/best_v2.pt")
    print(f"  Last checkpoint:   {ckpt_dir}/last_v2.pt")
